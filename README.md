...
## ⚡ 5. 기존 도구들과 비교 (Comparison)

| 비교 항목              | 기존 CLI 도구 (SSH)               | 기존 무거운 도구 (Grafana 등)        | **Docker Monitor (본 도구)**             |
| :--------------------- | :-------------------------------- | :----------------------------------- | :--------------------------------------- |
| **설치 편의성**        | 매번 SSH 터널링 및 비밀번호 필요  | 서버 측 에이전트 및 DB 구축 필수     | **설정 없음, 1개 단일 EXE로 바로 실행**  |
| **시스템 자원 사용량** | 낮음 (단, 시각화 불가)            | 높음 (서버 CPU/RAM 대량 소모)        | **매우 낮음 (초경량 비동기 통신)**       |
| **반응 속도**          | 명령을 칠 때만 수치 반환          | 주기적인 배치 폴링 (5~10초 지연)     | **실시간 양방향 소켓 스트리밍 (즉시)**   |
| **로그 백업**          | `docker logs > log.txt` 수동 처리 | 별도 로깅 시스템 구축 필요 (Loki 등) | **원격 타임스탬프 필터링 원클릭 백업**   |
| **네트워크 단절 대응** | 세션이 끊기면 명령어 재입력       | 지연 발생 혹은 화면 새로고침 필요    | **백그라운드 자가 복구 (무정지)** |
